...
const MODULES: ReadonlyArray<SceneModule> = [
  {
    ...
    text: "Alles, was du im Browser siehst und anklickst: Texte, Bilder, Buttons, Formulare. Das ist die sichtbare Oberfläche des Produkts.",
  },
  {
    key: "mobile",
    title: "App (Mobile)",
    text: "Dieselben Inhalte als App fürs Handy — angepasst an kleine Bildschirme, Touch und unterwegs.",
  },
  {
    key: "backend",
    title: "Backend (Maschinenraum)",
    text: "Der unsichtbare Motor. Hier wird gerechnet, geprüft und entschieden: Anmeldungen, Berechtigungen, Abläufe. Nutzer sehen ihn nie — ohne ihn läuft aber nichts.",
  },
  {
    key: "db",
    title: "Datenbank (Archiv)",
    text: "Der sichere Ort, an dem alle Daten dauerhaft liegen: Konten, Inhalte, Einstellungen. Beim nächsten Login ist alles wieder da.",
  },
  {
    key: "ops",
    title: "Deployment & Versionierung",
    text: "Diese Schicht liegt über allem — Website, App, Backend und Datenbank. Jede Änderung wird versioniert, sodass man jederzeit zu einer früheren, funktionierenden Version zurück kann. Updates gehen ohne Ausfall live (Deployment), das System wird laufend gewartet, und es gibt regelmäßige Backups, falls doch mal etwas schiefgeht.",
  },
];
...
const FLIP_MARGIN_X = 50;
const FLIP_MARGIN_Y = 50;
const FLIP_TARGET_W = 240;
const FLIP_TARGET_H = 130;
...
function select(key: ModuleKey) {
  ...
  const s = Math.min(FLIP_TARGET_W / bbox.width, FLIP_TARGET_H / bbox.height);
  const tx = FLIP_MARGIN_X - bbox.x * s;
  const ty = FLIP_MARGIN_Y - bbox.y * s;
...
function reset() {
  ...
  nextTick(() => {
    if (previous) moduleRefs.value[previous]?.focus();
  });
}
...
<template>
  ...
  <div
    ref="stageEl"
    class="stage"
    :class="{ 'detail-open': activeKey !== null }"
  ...
  >
    <div class="scene" :class="{ 'is-live': isLive }">
      <svg
        ...
        role="img"
        aria-label="Interaktive Darstellung eines Fullstack-Systems: Website, App, Backend, Datenbank und Deployment-Schicht"
      >
        ...
        <g
          ...
          role="button"
          aria-label="Deployment und Versionierung"
          @click="activate('ops')"
    ...
    <!-- Detail overlay — text appears under the flying icon (top-left corner).
         No close button: click the active icon again, or press Esc, to dismiss. -->
...
.conduit-glow {
  ...
  stroke-width: 3;
  stroke-linecap: round;
  /* Longer comet-trail with a small gap so the moving light reads clearly */
...
.scene.is-live .c-mobile .conduit-glow {
  animation-delay: -1.5s;
}
.scene.is-live .c-db .conduit-glow {
...
.scene.is-live .commit-head {
  animation: commit 3.4s ease-in-out infinite;
...
/* Tablet / desktop: text sits to the RIGHT of the icon, aligned to its top
   edge — feels more like a "detail page" layout. Icon ends at viewBox x≈290
   (≈24%); we start the text at 28% to leave a small gap. */
...
@media (prefers-reduced-motion: reduce) {
  ...
  /* Static reduced-motion packets are already rendered with cx/cy fallbacks
     by the template — no extra positioning needed here. */
  .module {
    opacity: 1;
    transform: none;
    transition: none;
